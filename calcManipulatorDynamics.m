function [torque1, torque2, torque3] = calcManipulatorDynamics(theta1, theta1_d, theta1_dd, theta2,  theta2_d,  theta2_dd, theta3,  theta3_d,  theta3_dd, theta0, theta0_d, theta0_dd)
    ...
    % A third rotation matrix is used to translate the counter force induced by
    % by the cart during balancing. The force will always be in the x-
    % direction in the Cartesian, and r23 will translate it to so it can be
    % expressed with respect to the frame of joint 2. This theta3 is the angle 
    % required to return the end effector back to its original orientation. 
    theta3 = theta1;
    ...
    % There is no v00, but will b e used to apply gravity to the system
    v00 = [0; g; 0];

    % Inertia Tensor (For this system)
    IT1_C = 0;
    IT2_C = 0;
    IT3_C = 0;
    IT4_C = 0;
    ...
    w11 = theta1_d * Z;
    w11_d = theta1_dd * Z;
    w22 = transpose(r12) * w11 + theta2_d * Z;
    w22_d = transpose(r12) * w11_d + cross(transpose(r12) * w11, theta2_d * Z) + theta2_dd * Z;
    w33 = transpose(r23) * w22 + theta3_d * Z;
    w33_d = transpose(r23) * w22_d + cross(transpose(r23) * w22, theta3_d * Z) + theta3_dd * Z;
    ...
    % f33 is the force applied to the linkage at the end of the 2nd joint. This
    % is included as the counter force required to balance the inverted
    % pendulum. In f22, the force is rotated by r23 so the force is applied
    % with respect to the frame of the 2nd link. 
